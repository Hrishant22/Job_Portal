          ...
          <div className='flex gap-2 mb-6 mt-3'>
            {
              ...
              ?<>
                <label className='flex items-center' htmlFor="resumeUpload">
                  <p className='bg-emerald-100 text-emerald-600 px-4 py-2 rounded-lg mr-2'>{resume? resume.name: "Select Resume"}</p>
                  <input id='resumeUpload' onChange={e => setResume(e.target.files[0])} accept='application/pdf' type="file" hidden/>
                  <img src={assets.profile_upload_icon} className='cursor-pointer' alt="" />
                </label>
                <button onClick={updateResume} className='cursor-pointer bg-gray-100 text-gray-500 border border-gray-300 rounded-lg px-4 py-2'>Save</button>
              </>
              :<div className='flex gap-2'>
                <a target='_blank' href={userData.resume} className='cursor-pointer bg-emerald-100 text-emerald-600 border border-emerald-300 px-4 py-2 rounded-lg'>
                  Resume
                </a>
                <button onClick={() => setIsEdit(true)} className='cursor-pointer bg-gray-100 text-gray-500 border border-gray-300 rounded-lg px-4 py-2'>
                  Edit
                </button>
              </div>
            }
